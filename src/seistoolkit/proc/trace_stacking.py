    ...
    for start in range(0, len(traces), tr_window):
        ...
        for hdr_name in STACK_MEAN_HDRS:
            merged_hdr[hdr_name] = round(
                float(np.mean([getattr(trace, hdr_name) for trace in hdr_block])),
...
def _data_stacking(
    data: np.ndarray, tr_window: int, method: Literal["mean", "median"] = "mean"
) -> np.ndarray:
    ...
    stack_func = {"mean": np.mean, "median": np.median}[method]
...
def trace_stacking(
    dataset: Dataset, tr_window: int = 2, method: Literal["mean", "median"] = "mean"
) -> None:
    ...
    stacked_data = _data_stacking(dataset.trace_data(), tr_window, method)
    merged_hdrs = _hdrs_merging(dataset.traces, tr_window)

    if len(merged_hdrs) != len(stacked_data):
        raise ValueError("Number of merged headers does not match stacked traces.")

    dataset.traces = [Trace(hdr, data) for hdr, data in zip(merged_hdrs, stacked_data)]

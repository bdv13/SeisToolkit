...
from stk.models import Dataset, Picks
...
class SeabedPicker:
    ...
    def _compute_attributes(self, section: np.ndarray) -> dict[str, np.ndarray]:
        """Calculate horizon tracking attributes."""
        return {
            "reflection_strength": envelope(section),
            "reflection_amplitude": reflection_amplitude(section, self.config.polarity),
